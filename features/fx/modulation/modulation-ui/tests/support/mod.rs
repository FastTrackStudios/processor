//! Shared fixture for the modulation editor's headless tests.
//!
//! Mounts `modulation_ui::control_view::App` — the exact Dioxus surface the
...
use modulation_ui::control_view::{App, ModUi};
...
/// - `ModUi`, the params + meters the editor consumes.
...
pub fn mount_sized(width: u32, height: u32) -> Fixture {
    mount_with(Arc::new(ModParams::default()), width, height)
}
...
pub fn mount_with(params: Arc<ModParams>, width: u32, height: u32) -> Fixture {
    let ui_state = Arc::new(ModUiState::default());
